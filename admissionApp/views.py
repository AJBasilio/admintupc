...
def applicant_perspective_pdf(request):
    if request.user.is_authenticated and request.user.is_applicant:
        if request.method == "POST":
            ...
            existing_pdf = PdfFileReader(open('../myProject/admissionApp/static/Media/slip.pdf', "rb"))
            ...
            output.addPage(page)
            output_stream = open("../myProject/admissionApp/static/Media/new_slip.pdf", "wb")
            output.write(output_stream)
            output_stream.close()

            with open('../myProject/admissionApp/static/Media/new_slip.pdf', 'rb') as pdf:
